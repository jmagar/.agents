...
Run `python3 scripts/fetch_comments.py` to fetch all comments and review threads on the PR. Store output for later verification.
...
```bash
python3 scripts/mark_resolved.py <thread_id_1> [thread_id_2] ...
```

**Example:**
```bash
python3 scripts/mark_resolved.py \
...
python3 scripts/fetch_comments.py | \
  python3 scripts/verify_resolution.py
```
