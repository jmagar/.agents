---
description: Download videos, playlists, or audio with yt-dlp/MeTube
argument-hint: "[--audio] [--local|--nas] <url> [url ...]"
allowed-tools: Bash
---

Download requested media immediately. Default route is NAS/MeTube; use `--local` to run local `yt-dlp`.

!`bash -c 'set -euo pipefail

args_string="${1:-}"
if [ -z "${args_string//[[:space:]]/}" ]; then
  echo "Usage: /yt-dlp [--audio] [--local|--nas] <url> [url ...]" >&2
  exit 2
fi
...
for token in "${tokens[@]}"; do
  case "$token" in
    ...
    --nas|--metube)
      ...
      ;;
    --help|-h)
      echo "Usage: /yt-dlp [--audio] [--local|--nas] <url> [url ...]"
      exit 0
      ;;
    http://*|https://*)
      urls+=("$token")
