...
- **Clarified the VM's official name.** The sandbox is officially the **`agent-os`** VM (container `agent-os-win11`, image `dockur/windows`) on host `dookie`. "Winbox" is retained as the historical nickname and remains the skill name; both `winbox` and `agent-os` are accepted trigger phrases.
