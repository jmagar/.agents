---
name: winbox
description: 'Use to drive Claude''s sandboxed Windows 11 VM — officially the `agent-os` VM, container `agent-os-win11` (`dockur/windows`) on host `dookie`, historically nicknamed "winbox" — via the Windows-MCP server installed inside it. Triggers: "agent-os", "the agent-os VM", "winbox", "the windows sandbox", "the dookie windows", "drive the windows VM", "spin up agent-os", "run X on agent-os", "screenshot agent-os", "PowerShell on agent-os", or explicit noVNC at http://dookie:8006. Prefer `mcp__windows-mcp__*` (App, Click, Type, Screenshot, Snapshot, Shell, Clipboard, FileSystem, Process, Registry, Shortcut, Notification, Wait, Move, Scroll, Scrape, MultiSelect, MultiEdit) over the legacy noVNC+agent-browser path — Windows-MCP is faster, has no Shift-key flakiness, and exposes interactive element ids via Snapshot. Does NOT fire on the user''s personal Windows on steamy-wsl — that target uses the `nircmd` skill. The sandbox is Claude''s alone: install software, change settings, run shells freely without confirmation.'
---

# winbox (the `agent-os` VM)

A real Windows 11 desktop reserved for Claude, running on host `dookie` as the **`agent-os`** VM (container name `agent-os-win11`, image `dockur/windows`). "Winbox" is the historical nickname for this sandbox and remains the skill name; the *official* name is **agent-os**. Both work as trigger phrases.
...
Windows-MCP is exposed inside the winbox over HTTP + Bearer token and registered in `~/.claude.json` as the `windows-mcp` server (Tailscale address; the bearer token lives in that config, not in this skill). Claude Code reaches it automatically — there is nothing to start.
...
```
Notification {"title": "winbox", "message": "winget install finished"}
```
